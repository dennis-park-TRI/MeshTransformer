    ...
    for image_file in image_list:
        if 'pred' not in image_file:
            ...
            visual_imgs_att = visualize_mesh_and_attention(
                renderer, batch_visual_imgs[0], pred_vertices[0].detach(), pred_vertices_sub2[0].detach(),
                pred_2d_431_vertices_from_smpl[0].detach(), pred_2d_joints_from_smpl[0].detach(), pred_camera.detach(),
                att[-1][0].detach()
            )
...
def parse_args():
    ...
    #########################################################
    parser.add_argument("--image_file_or_path", default='./test_images/human-body', type=str, help="test data")
    #########################################################
